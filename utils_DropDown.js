...
// Check if the dropdown_response.json file exists and read its content
...
async function answerDropDown(page) {
  // Define selector for dropdown questions
    ...
    if (questionTextElement) {
      ...
      if (!answer) {
        ...
        while (selectedValue === "Select an option") {
          await page.waitForTimeout(800);  // Wait for 500ms
      ...
      console.log("Question text element not found for the dropdown question.");
    }
  }
}
...
async function handleNewAnswerDropDown(questionText, page) {
  ...
  while (!answer) {
    answer = await new Promise((resolve) => {
      ...
      rl.question(`Please provide an answer for the dropdown question "${questionText}": `, (input) => {
        rl.close();
        resolve(input.trim());
      });
    });
