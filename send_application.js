...
const { answerDropDown, handleNewAnswerDropDown } = require('./utils_DropDown.js');
const { answerBinaryQuestions, handleNewQuestionBinary } = require('./utils_Binary');
const { answersDatabase, saveAnswer, handleNewQuestion, calculateSimilarity, getMostSimilarQuestion, getAnswer, normalizeAndTokenize } = require('./utils_Numeric.js');

//------------------------------------------------1.Numeric response HANDLER-------------------------

async function answerNumericQuestions(page) {
  try {
    // Select all question labels
    ...
    for (const questionElement of questionElements) {
      ...
      // Find the most similar question from the answers database
    ...
    console.error('Error in answering numeric questions:', error);
  }
}
...
async function handleNextOrReview(page) {
  ...
  while (hasNextButton) {
    try {
      ...
      if (nextButton) {
        ...
        await page.waitForTimeout(1000); // Wait for the next step to load
      ...
      hasNextButton = false; // Exit loop if any error occurs
    }
  }
  ...
  try {
    ...
    if (reviewButton) {
      ...
      if (submitButton) {
        ...
        if (attempts === maxAttempts) {
          console.error("Failed to click the Dismiss button after multiple attempts.");
        }
      } else {
    ...
    console.error('Error handling review and submit steps:', error);
  }
}
...
(async () => {
  ...
  try {
    ...
    //-----------------------------------1.Login-----------------------------------------------
    //1.Auto Login
    await page.fill('input[name="session_key"]', 'sz9448355011@gmail.com');
    await page.fill('input[name="session_password"]', 'zszszs');
    ...
    let currentPage = 1;
    let jobCounter = 0;
    const maxJobApplications = 50;

    while (jobCounter < maxJobApplications) {
      ...
      for (let job of jobListings) {
        if (jobCounter >= maxJobApplications) {
          console.log(`Reached the limit of ${maxJobApplications} job applications. Exiting.`);
          break;
        }
        ...
        try {
          easyApplyButton = await page.waitForSelector('button.jobs-apply-button', { timeout: 3000 });
        ...
        if (emailLabel) {
          ...
          await page.selectOption(`#${emailInputId}`, 'sz9448355011@gmail.com');
        }
